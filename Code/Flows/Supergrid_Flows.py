...
def export_XlinksEXT_Flows(my_network):
    ...
    HVDC0_out = pd.DataFrame(my_network.assets[12].flows.value[0:8760], columns = ['HVDC_GB-loc1'])
    HVDC0_in = pd.DataFrame(my_network.assets[12].flows.value[8760:17520], columns = ['HVDC_loc1-GB'])
    HVDC1_out = pd.DataFrame(my_network.assets[13].flows.value[0:8760], columns = ['HVDC_loc1-ZA'])
    HVDC1_in = pd.DataFrame(my_network.assets[13].flows.value[8760:17520], columns = ['HVDC_ZA-loc1'])
...
def export_XlinksEXT_costs_sizes(my_network):
    ...
    ### HVDC ###
    costs_Sizes.insert(0, 'GB-loc1_HVDC_GWp', [my_network.assets[12].asset_size()])
    costs_Sizes.insert(0, 'GB-loc1_HVDC_G$', [my_network.assets[12].cost.value])
    costs_Sizes.insert(0, 'loc1-ZA_HVDC_GWp', [my_network.assets[13].asset_size()])
    costs_Sizes.insert(0, 'loc1-ZA_HVDC_G$', [my_network.assets[13].cost.value])
