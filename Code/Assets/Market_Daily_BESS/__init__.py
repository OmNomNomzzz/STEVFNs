...
class Import_Asset(Asset_STEVFNs):
    ...
    def _update_parameters(self):
        ...
        full_profile = np.array(profile_df["Market Price"])
        ...
        final_profile = final_profile * 48
    ...
    def build_edge(self, edge_number):
        for counter1 in range(48):
            source_node_time = self.source_node_times[edge_number] + 0.5*counter1
            target_node_time = self.target_node_times[edge_number] + 0.5*counter1
...
class Export_Asset(Asset_STEVFNs):
    ...
    def _prevent_curtailment(self):
        for edge_number in range(self.number_of_edges):
            node_type = self.source_node_type_list[1]
            node_time = self.source_node_times[edge_number]
            node_location = self.source_node_location
            node = self.network.extract_node(node_location, node_type, node_time)
            node.curtailment = False
        return
    ...
    def build_edge(self, edge_number):
        #loop for 24 / 0.5 = 48 half hours per day
        for counter1 in range(48):
            source_node_time = self.source_node_times[edge_number] + 0.5*counter1
            target_node_time = self.target_node_times[edge_number] + 0.5*counter1
            new_edge = Edge_STEVFNs()
...
class Market_Daily_BESS_Asset(Multi_Asset):
    """Class for Battery Energy Storage System"""
    asset_name = "Market_Daily_BESS"
